...
size_t AudioBuffer::init() {
  ...
  if (psramInit() &&
      m_buffSizePSRAM >
          0) {  // PSRAM found, AudioBuffer will be allocated in PSRAM
    m_f_psram = true;
  ...
  if (m_buffer ==
      NULL) {  // PSRAM not found, not configured or not enough available
    m_f_psram = false;
    m_buffer =
        (uint8_t*)heap_caps_calloc(m_buffSizeRAM, sizeof(uint8_t),
...
uint8_t* AudioBuffer::getReadPtr() {
  ...
  if (len < m_maxBlockSize) {  // be sure the last frame is completed
    memcpy(m_endPtr, m_buffer,
           m_maxBlockSize - (len));  // cpy from m_buffer to m_endPtr with len
  }
  return m_readPtr;
}
...
void Audio::initInBuff() {
  if (!InBuff.isInitialized()) {
    ...
    if (size > 0) {
      AUDIO_INFO("PSRAM %sfound, inputBufferSize: %u bytes",
                 InBuff.havePSRAM() ? "" : "not ", size - 1);
    }
  }
...
#else
  // It is not necessary to call this function after i2s_driver_install() (it is
  // started automatically), however it is necessary to call it after i2s_stop()
...
void Audio::setDefaults() {
  ...
  _client = static_cast<WiFiClient*>(
      &client); /* default to *something* so that no NULL deref can happen */
  ts_parsePacket(0, 0, 0);  // reset ts routine
  if (m_lastM3U8host) {
    free(m_lastM3U8host);
    m_lastM3U8host = NULL;
  }
  AUDIO_INFO("buffers freed, free Heap: %lu bytes",
  ...
  m_f_tts = false;
  m_f_firstCall =
      true;  // InitSequence for processWebstream and processLocalFile
...
/*
    Text to speech API provides a speech endpoint based on our TTS
   (text-to-speech) model. More info:
   https://platform.openai.com/docs/guides/text-to-speech/text-to-speech

    Request body:
    model (string) [Required] - One of the available TTS models: tts-1 or
   tts-1-hd input (string) [Required] - The text to generate audio for. The
   maximum length is 4096 characters. voice (string) [Required] - The voice to
   use when generating the audio. Supported voices are alloy, echo, fable, onyx,
   nova, and shimmer. response_format (string) [Optional] - Defaults to mp3. The
   format to audio in. Supported formats are mp3, opus, aac, and flac. speed
   (number) [Optional] - Defaults to 1. The speed of the generated audio. Select
   a value from 0.25 to 4.0. 1.0 is the default.

    Usage: audio.openai_speech(OPENAI_API_KEY, "tts-1", input, "shimmer", "mp3",
   "1");
*/
bool Audio::openai_speech(const String& api_key, const String& model,
                          const String& input, const String& voice,
                          const String& response_format, const String& speed) {
  ...
  String post_body =
      "{"
      "\"model\": \"" +
      model + "\"," + "\"input\": \"" + input_clean + "\"," + "\"voice\": \"" +
      voice + "\"," + "\"response_format\": \"" + response_format + "\"," +
      "\"speed\": \"" + speed + "\"" + "}";

  String http_request =
      "POST " + String(path) +
      " HTTP/1.0\r\n"  // UNKNOWN ERROR CODE (0050) - crashing on HTTP/1.1 need
                       // to use HTTP/1.0
  ...
  if (res) {
    ...
    AUDIO_INFO("%s has been established in %lu ms, free Heap: %lu bytes", "SSL",
               (long unsigned int)dt, (long unsigned int)ESP.getFreeHeap());
    m_f_running = true;
  }
...
bool Audio::connecttoserver(const char* url, const char* api_key,
  ...
  if (res) {
    ...
    while (bytesSent < length) {
      size_t chunkSize = min((size_t)1024, length - bytesSent);
...
bool Audio::connecttohost(
  ...
  //        |   |                                     |    | | |   | |    | |
  //        (query string)
  //    ssl?|   |<-----host without extension-------->|port|<-----
  //    --extension----------->|<-first parameter->|<-second parameter->.......
  ...
  if (isalpha(host[pos_colon + 1])) pos_colon = -1;  // no portnumber follows
  pos_ampersand = indexOf(h_host, "&", 10);  // position of "&" in hostname

  if (pos_slash > 0) h_host[pos_slash] = '\0';

  if ((pos_colon > 0) &&
      ((pos_ampersand == -1) || (pos_ampersand > pos_colon))) {
  ...
  setDefaults();
  rqh = x_ps_calloc(lenHost + strlen(authorization) + 300,
  ...
  AUDIO_INFO("connect to: \"%s\" on port %d path \"/%s\"",
             h_host + hostwoext_begin, port, h_host + pos_slash + 1);
...
void Audio::showID3Tag(const char* tag, const char* value) {
  ...
  if (!strcmp(tag, "TP1"))
    sprintf(m_chbuf,
            "Lead artist(s)/Lead performer(s)/Soloist(s)/Performing group: %s",
            value);
  if (!strcmp(tag, "TP2"))
  ...
  if (!strcmp(tag, "TP4"))
    sprintf(m_chbuf, "Interpreted, remixed, or otherwise modified by: %s",
            value);
  ...
  if (!strcmp(tag, "TSS"))
    sprintf(m_chbuf, "Software/hardware and settings used for encoding: %s",
            value);
  if (!strcmp(tag, "TT1"))
...
bool Audio::latinToUTF8(char* buff, size_t bufflen, bool UTF8check) {
  // most stations send  strings in UTF-8 but a few sends in latin. To
  // standardize this, all latin strings are converted to UTF-8. If UTF-8 is
  // already present, nothing is done and true is returned. A conversion to
  // UTF-8 extends the string. Therefore it is necessary to know the buffer
  // size. If the converted string does not fit into the buffer, false is
  // returned
  ...
  // We cannot detect if a given string (or byte sequence) is a UTF-8 encoded
  // text as for example each and every series of UTF-8 octets is also a valid
  // (if nonsensical) series of Latin-1 (or some other encoding) octets. However
  // not every series of valid Latin-1 octets are valid UTF-8 series. So you can
  // rule out strings that do not conform to the UTF-8 encoding schema:

  if (UTF8check) {
    while (pos < len) {  // check first, if we have a clear UTF-8 string
      c = buff[pos];
      if (c >= 0xC2 &&
          c <= 0xDF) {  // may be 2 bytes UTF8, e.g. 0xC2B5 is 'µ' (MICRO SIGN)
        if (pos + 1 == len) {
          isUTF8 = false;
          break;
        }
      ...
      if (c >= 0xE0 && c <= 0xEF) {  // may  be 3 bytes UTF8, e.g. 0xE0A484 is
      ...
      if (c >=
          0xF0) {  // may  be 4 bytes UTF8, e.g. 0xF0919AA6 (TAKRI LETTER VA)
        if (pos + 3 >= len) {  //
          isUTF8 = false;
          break;
        }
...
int Audio::read_WAV_Header(uint8_t* data, size_t len) {
  ...
  if (m_controlCounter == 1) {
    m_controlCounter++;
    cs = (uint32_t)(*data + (*(data + 1) << 8) + (*(data + 2) << 16) +
                    (*(data + 3) << 24) - 8);
    headerSize += 4;
    return 4;  // ok
  }
  ...
  if (m_controlCounter == 5) {
    ...
    uint16_t nic =
        (uint16_t)(*(data + 2) +
                   (*(data + 3) << 8));  // Number of interleaved channels
    uint32_t sr =
        (uint32_t)(*(data + 4) + (*(data + 5) << 8) + (*(data + 6) << 16) +
                   (*(data + 7) << 24));  // Samplerate
    uint32_t dr =
        (uint32_t)(*(data + 8) + (*(data + 9) << 8) + (*(data + 10) << 16) +
                   (*(data + 11) << 24));  // Datarate
    uint16_t dbs =
        (uint16_t)(*(data + 12) + (*(data + 13) << 8));  // Data block size
    uint16_t bps =
        (uint16_t)(*(data + 14) + (*(data + 15) << 8));  // Bits per sample
  ...
  if (m_controlCounter == 8) {
    m_controlCounter++;
    size_t cs = *(data + 0) + (*(data + 1) << 8) + (*(data + 2) << 16) +
                (*(data + 3) << 24);  // read chunkSize
    headerSize += 4;
...
int Audio::read_FLAC_Header(uint8_t* data, size_t len) {
  ...
  if (m_controlCounter == FLAC_MBH) { /* METADATA_BLOCK_HEADER */
    ...
    FLACSetRawBlockParams(m_flacNumChannels, m_flacSampleRate,
                          m_flacBitsPerSample, m_flacTotalSamplesInStream,
                          m_audioDataSize);
    if (picLen) {
      size_t pos = audiofile.position();
      if (audio_id3image) audio_id3image(audiofile, picPos, picLen);
      audiofile.seek(pos);  // the filepointer could have been changed by the
                            // user, set it back
    }
    AUDIO_INFO("Audio-Length: %u", m_audioDataSize);
    retvalue = 0;
  ...
  if (m_controlCounter == FLAC_SINFO) { /* Stream info block */
    ...
    if (m_flacTotalSamplesInStream) {
      AUDIO_INFO("total samples in stream: %lu",
                 (long unsigned int)m_flacTotalSamplesInStream);
    } else {
      AUDIO_INFO("total samples in stream: N/A");
    }
    if (bps != 0 && m_flacTotalSamplesInStream) {
      AUDIO_INFO("audio file duration: %lu seconds",
                 (long unsigned int)m_flacTotalSamplesInStream /
                     (long unsigned int)m_flacSampleRate);
    }
    m_controlCounter = FLAC_MBH;  // METADATA_BLOCK_HEADER
    retvalue = l + 3;
    headerSize += retvalue;
...
int Audio::read_ID3_Header(uint8_t* data, size_t len) {
  ...
  static size_t
      totalId3Size;  // if we have more header, id3_1_size + id3_2_size + ....
  ...
  if (m_controlCounter == 0) { /* read ID3 tag and ID3 header size */
    ...
    id3Size =
        bigEndian(data + 6, 4,
                  7);  //  ID3v2 size  4 * %0xxxxxxx (shift left seven times!!)
    id3Size += 10;
  ...
  if (m_controlCounter == 4) {  // get the frame size
    ...
    compressed = (*(data + 5)) &
                 0x80;  // Frame is compressed using [#ZLIB zlib] with 4 bytes
                        // for 'decompressed size' appended to the frame header.
    remainingHeaderBytes--;
  ...
  // - -
  if (m_controlCounter ==
      5) {  // If the frame is larger than 512 bytes, skip the rest
    if (framesize > len) {
      framesize -= len;
      remainingHeaderBytes -= len;
      return len;
  ...
  if (m_controlCounter == 6) {           // Read the value
    ...
    // 0x80). $01 – UCS-2 encoded Unicode with BOM (Byte Order Mark), in ID3v2.2
    // and ID3v2.3. $02 – UTF-16BE encoded Unicode without BOM (Byte Order Mark)
    // , in ID3v2.4. $03 – UTF-8 encoded Unicode, in ID3v2.4.

    if (startsWith(tag, "APIC")) {  // a image embedded in file, passing it to
                                    // external function
                                    //    if(m_dataMode == AUDIO_LOCALFILE) {
      APIC_pos[numID3Header] = totalId3Size + id3Size - remainingHeaderBytes;
      APIC_size[numID3Header] = framesize;
      //    log_e("APIC_pos %i APIC_size %i", APIC_pos[numID3Header],
      //    APIC_size[numID3Header]);
      //    }
      return 0;
    }

    if (  // any lyrics embedded in file, passing it to external function
        startsWith(tag, "SYLT") || startsWith(tag, "TXXX") ||
        startsWith(tag, "USLT")) {
      if (m_dataMode == AUDIO_LOCALFILE) {
        SYLT_seen = true;
    ...
    if (encodingByte == 1 && dataLength > 1) {  // UTF16 with BOM
      bool big_endian = static_cast<unsigned char>(m_ibuff[0]) == 0xFE &&
                        static_cast<unsigned char>(m_ibuff[1]) == 0xFF;
      ...
      for (size_t i = data_start; i < dataLength; i += 2) {
        char16_t wchar;
        if (big_endian)
          wchar = (static_cast<unsigned char>(m_ibuff[i]) << 8) |
                  static_cast<unsigned char>(m_ibuff[i + 1]);
        else
          wchar = (static_cast<unsigned char>(m_ibuff[i + 1]) << 8) |
                  static_cast<unsigned char>(m_ibuff[i]);
    ...
    if (encodingByte == 2 && dataLength > 1) {  // UTF16BE
      ...
      for (size_t i = 0; i < dataLength; i += 2) {
        char16_t wchar = (static_cast<unsigned char>(m_ibuff[i]) << 8) |
                         static_cast<unsigned char>(m_ibuff[i + 1]);
        utf16_string.push_back(wchar);
      }
  ...
  if (m_controlCounter ==
      10) {  // frames in V2.2, 3bytes identifier, 3bytes size descriptor
    ...
    if (startsWith(tag, "PIC")) {  // image embedded in header
      if (m_dataMode == AUDIO_LOCALFILE) {
        ...
        if (m_f_Log)
          log_i("Attached picture seen at pos %d length %d", APIC_pos[0],
                APIC_size[0]);
      }
      ...
      if (m_dataMode == AUDIO_LOCALFILE) {
        ...
        if (m_f_Log)
          log_i("Attached lyrics seen at pos %d length %d", SYLT_pos,
                SYLT_size);
      }
    } else {
  ...
  if (m_controlCounter == 99) {  //  exist another ID3tag?
    ...
    if ((*(data + 0) == 'I') && (*(data + 1) == 'D') && (*(data + 2) == '3')) {
      ...
      m_audioDataSize = m_contentlength - m_audioDataStart;
      if (!m_f_m3u8data) AUDIO_INFO("Audio-Length: %u", m_audioDataSize);
      if (APIC_pos[0] && audio_id3image) {  // if we have more than one APIC,
                                            // output the first only
        size_t pos = audiofile.position();
...
int Audio::read_M4A_Header(uint8_t* data, size_t len) {
  ...
  auto parse_variant_length = [](uint8_t*& ptr) -> int {
    ...
    do {
      length = (length << 7) |
               (*ptr & 0x7F);  // Read the lower 7 bits of the current byte
  ...
  if (m_controlCounter == M4A_FTYP) { /* check_m4a_file */
    ...
    if ((m4a != 8) && (isom != 8) && (mp42 != 8)) {
      log_e("subtype 'MA4 ', 'isom' or 'mp42' expected, but found '%s '",
            (data + 8));
      stopSong();
      return -1;
    }
  ...
  if (m_controlCounter == M4A_TRAK) {  // trak
    if (specialIndexOf(data, "esds", len) > 0) {
      int esds = specialIndexOf(data, "esds",
                                len);  // Packaging/Encapsulation And Setup Data
      ...
      AUDIO_INFO("Sampling Frequency: %lu",
                 (long unsigned int)samplingFrequencies[sRate]);
    ...
    if (specialIndexOf(data, "mp4a", len) > 0) {
      ...
      int channel =
          bigEndian(data + offset + 20,
                    2);  // audio parameter must be set before starting
      int bps =
          bigEndian(data + offset + 22,
                    2);  // the aac decoder. There are RAW blocks only in m4a
  ...
  if (m_controlCounter == M4A_ILST) {  // ilst
    const char info[12][6] = {"nam\0", "ART\0",  "alb\0",  "too\0",
                              "cmt\0", "wrt\0",  "tmpo\0", "trkn\0",
                              "day\0", "cpil\0", "aART\0", "gen\0"};
    int offset = 0;
    // If it's a local file, the metadata has already been read, even if it
    // comes after the audio block. In the event that they are in front of the
    // audio block in a web stream, read them now
    if (!m_f_m4aID3dataAreRead) {
      for (int i = 0; i < 12; i++) {
        offset =
            specialIndexOf(data, info[i], len, true);  // seek info[] with '\0'
        if (offset > 0) {
          offset += 19;
  ...
  if (m_controlCounter == M4A_AMRDY) {  // almost ready
    ...
    if (m_dataMode == AUDIO_LOCALFILE) {
      m_contentlength =
          headerSize +
          m_audioDataSize;  // after this mdat atom there may be other atoms
...
size_t Audio::process_m3u8_ID3_Header(uint8_t* packet) {
  ...
  // "com.apple.streaming.transportStreamTimestamp" a time stamp is expected in
  // the header.
...
uint32_t Audio::stopSong() {
  ...
  if (audiofile) {
    // added this before putting 'm_f_localfile = false' in stopSong(); shoulf
    // never occur....
...
bool Audio::pauseResume() {
  ...
  if (m_dataMode == AUDIO_LOCALFILE || m_streamType == ST_WEBSTREAM ||
    ...
    if (!m_f_running) {
      memset(m_outBuff, 0,
             m_outbuffSize * sizeof(int16_t));  // Clear OutputBuffer
      m_validSamples = 0;
    }
  }
...
#if (ESP_IDF_VERSION_MAJOR == 5)
  err = i2s_channel_write(m_i2s_tx_handle, (int16_t*)m_outBuff + count,
                          validSamples * sampleSize, &i2s_bytesConsumed, 10);
#else
  err = i2s_write((i2s_port_t)m_i2s_num, (int16_t*)m_outBuff + count,
                  validSamples * sampleSize, &i2s_bytesConsumed, 10);
#endif
...
exit:
  if (err == ESP_OK)
    ...
    log_e(
        "Writing timeout, no writing event received from ISR within "
        "ticks_to_wait");
  ...
  if (m_playlistFormat != FORMAT_M3U8) {  // normal process
    ...
    switch (m_dataMode) {
      ...
      case AUDIO_DATA:
        ...
        if (m_f_continue) {  // at this point m_f_continue is true, means
...
bool Audio::readPlayListData() {
  ...
  // reads the content of the playlist and stores it in the vector
  // m_contentlength m_contentlength is a table of pointers to the lines
  ...
  while (true) {  // outer while
    ...
    if (!m_f_psramFound && m_playlistContent.size() == 101) {
      AUDIO_INFO(
          "the number of lines in playlist > 100, for bigger playlist use "
          "PSRAM!");
      break;
    }
    if (m_playlistContent.size() && m_playlistContent.size() % 1000 == 0) {
      AUDIO_INFO("current playlist line: %lu",
                 (long unsigned)m_playlistContent.size());
    }
    // termination conditions
    // 1. The http response header returns a value for contentLength -> read
    // chars until contentLength is reached
    // 2. no contentLength, but Transfer-Encoding:chunked -> compute chunksize
    // and read until chunksize is reached
    // 3. no chunksize and no contentlengt, but Connection: close -> read all
    // available chars
    if (ctl == m_contentlength) {
  ...
  for (int i = 0; i < lines;
       i++) {  // print all string in first vector of 'arr'
...
const char* Audio::parsePlaylist_M3U() {
  ...
  for (int i = 0; i < lines; i++) {
    ...
    pos =
        indexOf(m_playlistContent[i], "http://:@", 0);  // ":@"??  remove that!
    if (pos >= 0) {
      AUDIO_INFO("Entry in playlist found: %s",
                 (m_playlistContent[i] + pos + 9));
...
const char* Audio::parsePlaylist_PLS() {
  ...
  for (int i = 0; i < lines; i++) {
    if (i == 0) {
      ...
      if (strcmp(m_playlistContent[0], "[playlist]") !=
          0) {                              // first entry in valid pls
    ...
    if (startsWith(m_playlistContent[i], "File1")) {
      ...
      pos = indexOf(m_playlistContent[i], "http",
                    0);  // File1=http://streamplus30.leonex.de:14840/;
      if (pos >= 0) {    // yes, URL contains "http"?
        host = m_playlistContent[i] +
               pos;  // Now we have an URL for a stream in host.
      }
      continue;
    }
    ...
    if (indexOf(m_playlistContent[i], "Invalid username") >=
        0) {      // Unable to access account:
...
const char* Audio::parsePlaylist_ASX() {  // Advanced Stream Redirector
  ...
  for (int i = 0; i < lines; i++) {
    ...
    if (f_entry) {
      if (indexOf(m_playlistContent[i], "ref href") >
          0) {  //  <ref href="http://87.98.217.63:24112/stream" />
        pos = indexOf(m_playlistContent[i], "http", 0);
        if (pos > 0) {
          host = (m_playlistContent[i] +
                  pos);  // http://87.98.217.63:24112/stream" />
          int pos1 =
              indexOf(host, "\"", 0);  // http://87.98.217.63:24112/stream
          if (pos1 > 0)
    ...
    if (pos >= 0) {
      char* plsStationName =
          (m_playlistContent[i] + pos + 7);  // remove <Title>
    ...
    if (indexOf(m_playlistContent[i], "http") == 0 &&
        !f_entry) {  // url only in asx
      host = m_playlistContent[i];
    }
  }
  return host;
...
const char* Audio::parsePlaylist_M3U8() {
  ...
  char llasc[21];  // uint64_t max = 18,446,744,073,709,551,615  thats 20 chars
                   // + \0
  ...
  if (lines) {
    for (uint16_t i = 0; i < lines; i++) {
      ...
      // "#EXT-X-DISCONTINUITY-SEQUENCE: // not used, 0: seek for continuity
      // numbers, is sometimes not set
      ...
      if (startsWith(m_playlistContent[i], "#EXTINF")) {
        ...
        if (!startsWith(m_playlistContent[i], "http")) {
          ...
          if (m_lastM3U8host != 0) {
            tmp = (char*)malloc(strlen(m_lastM3U8host) +
                                strlen(m_playlistContent[i]) + 1);
            strcpy(tmp, m_lastM3U8host);
          } else {
            tmp = (char*)malloc(strlen(m_lastHost) +
                                strlen(m_playlistContent[i]) + 1);
          ...
          if (m_playlistContent[i][0] != '/') {
            ...
            //  chunklist:  ddd.aac                              //
            //  m_playlistContent[i] result: http://station.com/aaa/bbb/ddd.aac
            //  // m_playlistContent[i]
    ...
    vector_clear_and_shrink(
        m_playlistContent);  // clear after reading everything,
                             // m_playlistContent.size is now 0
  }
      ...
      if (f_mediaSeq_found) {
        ...
        if (mediaSeq < xMedSeq) {
          ...
          if (mediaSeq != UINT64_MAX) {
            log_e("err, %u packets lost from %u, to %u", mediaSeq - xMedSeq,
                  xMedSeq, mediaSeq);
          }
          xMedSeq = mediaSeq;
        }
...
const char* Audio::m3u8redirection(uint8_t* codec) {
  ...
  const char codecString[9][11] = {
      ...
      "mp4a.40.05",  // MPEG-4 HE-AAC v1 (AAC LC + SBR), leading 0 for Aud-OTI
                     // compatibility
  ...
  // if((!endsWith(m_playlistContent[choosenLine], "m3u8") &&
  // indexOf(m_playlistContent[choosenLine], "m3u8?") == -1)) {
  //     // we have a new m3u8 playlist, skip to next line
  //     int pos = indexOf(m_playlistContent[choosenLine - 1], "CODECS=\"mp4a",
  //     18); if(pos < 0) { // not found
  //         int pos1 = indexOf(m_playlistContent[choosenLine - 1], "CODECS=",
  //         18); if(pos1 < 0) pos1 = 0; log_e("codec %s in m3u8 playlist not
  //         supported", m_playlistContent[choosenLine - 1] + pos1); goto exit;
  //     }
  // }
  ...
  if (!startsWith(m_playlistContent[choosenLine], "http")) {
    ...
    // http://livees.com/prog_index48347.aac http://livees.com/prog_index.m3u8
    // and chunklist022.m3u8 --> http://livees.com/chunklist022.m3u8

    tmp = (char*)malloc(strlen(m_lastHost) +
                        strlen(m_playlistContent[choosenLine]));
    strcpy(tmp, m_lastHost);
  ...
  if (startsWith(m_playlistContent[choosenLine], "../")) {
    ...
    while (startsWith(m_playlistContent[choosenLine], "../")) {
      memcpy(m_playlistContent[choosenLine], m_playlistContent[choosenLine] + 3,
             strlen(m_playlistContent[choosenLine] + 3) + 1);  // shift << 3
  ...
  return m_playlistContent[choosenLine];  // it's a redirection, a new m3u8
                                          // playlist
}
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
uint64_t
Audio::m3u8_findMediaSeqInURL() {  // We have no clue what the media sequence is
  ...
  // log_i("m_playlistContent[linesWithURL[0]] %s",
  // m_playlistContent[linesWithURL[0]]);
  // log_i("m_playlistContent[linesWithURL[1]] %s",
  // m_playlistContent[linesWithURL[1]]);
  // log_i("m_playlistContent[linesWithURL[2]] %s",
  // m_playlistContent[linesWithURL[2]]);
  ...
  for (int16_t pos = len; pos >= 0; pos--) {
    if (isdigit(m_playlistContent[linesWithURL[0]][pos])) {
      ...
      lltoa(b, llasc, 10);
      int16_t idx_b =
          indexOf(m_playlistContent[linesWithURL[1]], llasc, pos - 1);
      lltoa(c, llasc, 10);
      int16_t idx_c =
          indexOf(m_playlistContent[linesWithURL[2]], llasc, pos - 1);
      if (idx_b > 0 && idx_c > 0 && idx_b - pos < 3 &&
          idx_c - pos < 3) {  // idx_b and idx_c must be positive and near pos
        MediaSeq = a;
...
bool Audio::STfromEXTINF(char* str) {
  ...
  // orig: #EXTINF:10,title="text=\"Spot Block End\"
  // amgTrackId=\"9876543\"",artist=" ",url="length=\"00:00:00\"" conv:
  // StreamTitle=text=\"Spot Block End\" amgTrackId=\"9876543\" -
...
void Audio::processLocalFile() {
  ...
  const uint32_t maxFrameSize =
      InBuff.getMaxBlockSize();  // every mp3/aac frame is not bigger
  ...
  int32_t bytesAddedToBuffer =
      audiofile.read(InBuff.getWritePtr(), availableBytes);
  ...
  if (!m_f_stream) {
    ...
    if (m_controlCounter != 100) {
      ...
      if (InBuff.bufferFilled() > maxFrameSize ||
          (InBuff.bufferFilled() ==
           m_fileSize)) {  // at least one complete frame or the file is smaller
  ...
  if (m_resumeFilePos >= 0) {
    ...
    m_f_lockInBuffer = true;  // lock the buffer, the InBuffer must not be
                              // re-entered in playAudioData()
    while (m_f_audioTaskIsDecoding)
      vTaskDelay(
          1);  // We can't reset the InBuffer while the decoding is in progress
  ...
  // end of file reached? - - - - - - - - - - - - - - - - - - - - - - - - - - -
  // - - - - - - - - - - - - - - - - - -
  if (m_f_eof) {     // m_f_eof and m_f_ID3v1TagFound will be set in
                     // playAudioData()
...
void Audio::processWebStream() {
  ...
  // - - - - - - - - - - - - - - - - - - -
  if (InBuff.bufferFilled() > maxFrameSize &&
      !m_f_stream) {             // waiting for buffer filled
    if (m_codec == CODEC_OGG) {  // log_i("determine correct codec here");
      uint8_t codec = determineOggCodec(InBuff.getReadPtr(), maxFrameSize);
      if (codec == CODEC_FLAC) {
        initializeDecoder(codec);
...
void Audio::processWebFile() {
  ...
  if (m_controlCounter != 100) {
    if (InBuff.bufferFilled() > maxFrameSize ||
        (InBuff.bufferFilled() ==
         m_contentlength)) {  // at least one complete frame or the file is
                              // smaller
...
void Audio::processWebStreamTS() {
  ...
  if (availableBytes) {
    ...
    if (m_f_chunked) chunkSize = chunkedDataTransfer(&readedBytes);
    int res =
        _client->read(ts_packet + ts_packetPtr, ts_packetsize - ts_packetPtr);
    if (res > 0) {
      ts_packetPtr += res;
      byteCounter += res;
      ...
      if (f_firstPacket) {  // search for ID3 Header in the first packet
        ...
        if (ID3_HeaderSize) {
          memcpy(ts_packet, &ts_packet[ID3_HeaderSize],
                 ts_packetsize - ID3_HeaderSize);
  ...
  if (availableBytes) {  // an ID3 header could come here
    ...
    if (firstBytes) {
      if (ID3WritePtr < ID3BuffSize) {
        ID3WritePtr += _client->readBytes(&ID3Buff[ID3WritePtr],
                                          ID3BuffSize - ID3WritePtr);
        return;
      }
      if (m_controlCounter < 100) {
        int res =
            read_ID3_Header(&ID3Buff[ID3ReadPtr], ID3BuffSize - ID3ReadPtr);
      ...
      if (ws >= ID3BuffSize - ID3ReadPtr) {
        memcpy(InBuff.getWritePtr(), &ID3Buff[ID3ReadPtr],
               ID3BuffSize - ID3ReadPtr);
        InBuff.bytesWritten(ID3BuffSize - ID3ReadPtr);
      } else {
        memcpy(InBuff.getWritePtr(), &ID3Buff[ID3ReadPtr], ws);
        InBuff.bytesWritten(ws);
        memcpy(InBuff.getWritePtr(), &ID3Buff[ws + ID3ReadPtr],
               ID3BuffSize - (ID3ReadPtr + ws));
    ...
    if (InBuff.writeSpace() >= availableBytes) {
      ...
      bytesWasWritten =
          _client->read(InBuff.getWritePtr(), InBuff.writeSpace());
    }
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
bool Audio::parseHttpResponseHeader() {  // this is the response to a GET /
                                         // request
  ...
  while (true) {  // outer while
    ...
    while (_client->available()) {
      ...
      if (b == '\n') {
        if (!pos) {  // empty line received, is the last line of this
                     // responseHeader
          if (ct_seen)
            goto lastToDo;
          else
    ...
    int16_t posColon =
        indexOf(rhl, ":", 0);  // lowercase all letters up to the colon
    if (posColon >= 0) {
    ...
    if (startsWith(rhl, "HTTP/")) {  // HTTP status error code
        ...
        if (strcmp(c_host, m_lastHost) != 0) {  // prevent a loop
          ...
          if (pos_slash > 9) {
            if (!strncmp(c_host, m_lastHost, pos_slash)) {
              AUDIO_INFO("redirect to new extension at existing host \"%s\"",
                         c_host);
              if (m_playlistFormat == FORMAT_M3U8) {
      ...
      int pos1, pos2;  // pos3;
      // e.g we have this headerline:  content-disposition: attachment;
      // filename=stream.asx filename is: "stream.asx"
      ...
      int32_t br =
          atoi(c_bitRate);  // Found bitrate tag, read the bitrate in Kbit
      br = br * 1000;
      setBitrate(br);
      ...
      latinToUTF8(
          rhl,
          sizeof(
              rhl));  // if already UTF-8 do nothing, otherwise convert to UTF-8
      if (strlen(c_idesc) > 0 &&
          specialIndexOf((uint8_t*)c_idesc, "24bit", 0) > 0) {
      ...
      if (endsWith(rhl, "chunked") ||
          endsWith(rhl, "Chunked")) {  // Station provides chunked transfer
        m_f_chunked = true;
  ...
  switch (codec) {
    case CODEC_MP3:
      if (!MP3Decoder_IsInit()) {
        ...
        AUDIO_INFO(
            "MP3Decoder has been initialized, free Heap: %lu bytes , free "
            "stack %lu DWORDs",
            (long unsigned int)gfH, (long unsigned int)hWM);
    ...
    case CODEC_AAC:
      if (!AACDecoder_IsInit()) {
        ...
        hWM = uxTaskGetStackHighWaterMark(NULL);
        AUDIO_INFO(
            "AACDecoder has been initialized, free Heap: %lu bytes , free "
            "stack %lu DWORDs",
            (long unsigned int)gfH, (long unsigned int)hWM);
    ...
    case CODEC_FLAC:
      ...
      AUDIO_INFO(
          "FLACDecoder has been initialized, free Heap: %lu bytes , free stack "
          "%lu DWORDs",
          (long unsigned int)gfH, (long unsigned int)hWM);
      break;
    case CODEC_OPUS:
      ...
      hWM = uxTaskGetStackHighWaterMark(NULL);
      AUDIO_INFO(
          "OPUSDecoder has been initialized, free Heap: %lu bytes , free stack "
          "%lu DWORDs",
          (long unsigned int)gfH, (long unsigned int)hWM);
    ...
    case CODEC_VORBIS:
      ...
      hWM = uxTaskGetStackHighWaterMark(NULL);
      AUDIO_INFO(
          "VORBISDecoder has been initialized, free Heap: %lu bytes,  free "
          "stack %lu DWORDs",
          (long unsigned int)gfH, (long unsigned int)hWM);
...
void Audio::showstreamtitle(const char* ml) {
  // example for ml:
  // StreamTitle='Oliver Frank - Mega
  // Hitmix';StreamUrl='www.radio-welle-woerthersee.at'; or
  // adw_ad='true';durationMilliseconds='10135';adId='34254';insertionType='preroll';
  ...
  if (idx1 >= 0) {
    if (indexOf(ml, "xml version=", 7) > 0) {
      /* e.g. xmlStreamTitle
            StreamTitle='<?xml version="1.0"
         encoding="utf-8"?><RadioInfo><Table><DB_ALBUM_ID>37364</DB_ALBUM_ID>
            <DB_ALBUM_IMAGE>00000037364.jpg</DB_ALBUM_IMAGE><DB_ALBUM_NAME>Boyfriend</DB_ALBUM_NAME>
            <DB_ALBUM_TYPE>Single</DB_ALBUM_TYPE><DB_DALET_ARTIST_NAME>DOVE
         CAMERON</DB_DALET_ARTIST_NAME>
            ...
            <DB_LEAD_ARTIST_NAME>Dove
         Cameron</DB_LEAD_ARTIST_NAME><DB_RADIO_IMAGE>cidadefm.jpg</DB_RADIO_IMAGE>
            ...
            </END_TIME_UTC><SHOW_NAME>Cidade</SHOW_NAME><SHOW_HOURS>22h às
         07h</SHOW_HOURS><SHOW_PANEL>0</SHOW_PANEL>
...
void Audio::showCodecParams() {
  ...
  if (m_codec == CODEC_AAC) {
    ...
    if (answ > 0 && answ < 4) {
      const char sbr[4][50] = {
          "without SBR", "upsampled SBR", "downsampled SBR",
          "no SBR used, but file is upsampled by a factor 2"};
...
int Audio::findNextSync(uint8_t* data, size_t len) {
  // Mp3 and aac audio data are divided into frames. At the beginning of each
  // frame there is a sync word. The sync word is 0xFFF. This is followed by
  // information about the structure of the frame. Wav files have no frames
...
void Audio::setDecoderItems() {
  ...
  if (m_codec == CODEC_FLAC) {
    ...
    if (FLACGetAudioDataStart() > 0) {  // only flac-ogg, native flac sets
                                        // audioDataStart in readFlacHeader()
...
int Audio::sendBytes(uint8_t* data, size_t len) {
  ...
  if (bytesDecoded == 0 && m_decodeError == 0) {  // unlikely framesize
    ...
    m_f_playing = false;  // seek for new syncword
    // we're here because there was a wrong sync word so skip one byte and seek
    // for the next
    return 1;
  }
  ...
  switch (m_codec) {
    case CODEC_WAV:
      if (getBitsPerSample() == 16) {
        memmove(m_outBuff, data, len);  // copy len data in outbuff and set
                                        // validsamples and bytesdecoded=len
    ...
    case CODEC_FLAC:
      ...
      if (vec.size() > 0) {  // get blockpic data
        ...
        // for(int i = 0; i < vec.size(); i += 2) { log_i("segment %02i, pos
        // %07i, len %05i", i / 2, vec[i], vec[i + 1]); }
        // log_i("---------------------------------------------------------------------------");
...
void Audio::computeAudioTime(uint16_t bytesDecoderIn,
  ...
  if (timeStamp + 500 < millis()) {
    ...
    uint32_t bitRate =
        ((deltaBytesIn * 8000) /
         delta_t);  // we know the time and bytesIn to compute the bitrate
...
uint32_t Audio::getTotalPlayingTime() {
  // Is set to zero by a connectToXXX() and starts as soon as the first audio
  // data is available, the time counting is not interrupted by a 'pause /
  // resume' and is not reset by a fileloop
  return millis() - m_PlayingStartTime;
}
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
bool Audio::setTimeOffset(
    int sec) {  // fast forward or rewind the current position in seconds
...
bool Audio::audioFileSeek(const float speed) {
  ...
  //     i2s_channel_reconfig_std_clock(m_i2s_tx_handle,
  //     &m_i2s_std_cfg.clk_cfg); I2Sstart(m_i2s_num);
  // #else
...
#else
  m_i2s_config.channel_format = I2S_CHANNEL_FMT_RIGHT_LEFT;
  i2s_set_clk((i2s_port_t)m_i2s_num, m_sampleRate, I2S_BITS_PER_SAMPLE_16BIT,
              I2S_CHANNEL_STEREO);
#endif
  memset(m_filterBuff, 0, sizeof(m_filterBuff));  // Clear FilterBuffer
  IIR_calculateCoefficients(
      m_gain0, m_gain1,
      m_gain2);  // must be recalculated after each samplerate change
  return;
}
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
void Audio::setI2SCommFMT_LSB(bool commFMT) {
  // false: I2S communication format is by default I2S_COMM_FORMAT_I2S_MSB,
  // right->left (AC101, PCM5102A) true:  changed to I2S_COMM_FORMAT_I2S_LSB for
  // some DACs (PT8211)
  ...
  if (commFMT) {
    ...
    m_i2s_config.communication_format =
        (i2s_comm_format_t)(I2S_COMM_FORMAT_STAND_MSB);
  } else {
...
void Audio::computeVUlevel(int16_t sample[2]) {
  ...
  auto avg = [&](uint8_t* sampArr) {  // lambda, inner function, compute the
  ...
  auto largest = [&](uint8_t* sampArr) {  // lambda, inner function, compute the
                                          // largest of 8 samples
    uint16_t maxValue = 0;
    for (int i = 0; i < 8; i++) {
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void Audio::setTone(int8_t gainLowPass, int8_t gainBandPass,
                    int8_t gainHighPass) {
  ...
  /*
        ...
        Because when the EQ is adjusted, the IIR filter will be cleared and
     played, mixed in the audio data frame, and a click-like sound will be
     produced.
...
void Audio::setVolume(
    uint8_t vol,
    uint8_t curve) {  // curve 0: default, curve 1: flat at the beginning

  uint16_t v = ESP_ARDUINO_VERSION_MAJOR * 100 +
               ESP_ARDUINO_VERSION_MINOR * 10 + ESP_ARDUINO_VERSION_PATCH;
  if (v < 207)
    AUDIO_INFO("Do not use this ancient Adruino version V%d.%d.%d",
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void Audio::computeLimit() {   // is calculated when the volume or balance
                               // changes
  ...
  switch (m_curve) {
    ...
    case 1:  // logarithmic
      ...
      if (m_vol > 0) {
        v = m_vol *
            ((std::exp(log1 + (m_vol - 1) * (std::log(m_vol_steps) - log1) /
                                  (m_vol_steps - 1))) /
             m_vol_steps) /
            m_vol_steps;
      } else {
...
void Audio::Gain(int16_t* sample) {
  /* important: these multiplications must all be signed ints, or the result
   * will be invalid */
...
void Audio::IIR_calculateCoefficients(
    int8_t G0, int8_t G1,
    int8_t G2) {  // Infinite Impulse Response (IIR) filters
  ...
  if (getSampleRate() <
      FcHS * 2 - 100) {  // Prevent HighShelf filter from clogging
    FcHS = getSampleRate() / 2 - 100;
    // according to the sampling theorem, the sample rate must be at least 2 *
    // 6000 >= 12000Hz for a filter frequency of 6000Hz. If this is not the
    // case, the filter frequency (plus a reserve of 100Hz) is lowered
    AUDIO_INFO("Highshelf frequency lowered, from 6000Hz to %luHz",
               (long unsigned int)FcHS);
  }
  float K, norm, Q, Fc, V;
  ...
  //    log_i("LS a0=%f, a1=%f, a2=%f, b1=%f, b2=%f", m_filter[0].a0,
  //    m_filter[0].a1, m_filter[0].a2,
  //                                                  m_filter[0].b1,
  //                                                  m_filter[0].b2);
  //    log_i("EQ a0=%f, a1=%f, a2=%f, b1=%f, b2=%f", m_filter[1].a0,
  //    m_filter[1].a1, m_filter[1].a2,
  //                                                  m_filter[1].b1,
  //                                                  m_filter[1].b2);
  //    log_i("HS a0=%f, a1=%f, a2=%f, b1=%f, b2=%f", m_filter[2].a0,
  //    m_filter[2].a1, m_filter[2].a2,
  //                                                  m_filter[2].b1,
  //                                                  m_filter[2].b2);
}
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
bool Audio::ts_parsePacket(uint8_t* packet, uint8_t* packetStart,
  ...
  // --------------------------------------------------------------------------------------------------------
  // 0. Byte SyncByte  | 0 | 1 | 0 | 0 | 0 | 1 | 1 | 1 | always bit pattern of
  // 0x47
  //---------------------------------------------------------------------------------------------------------
  ...
  // PUSI Payload unit start indicator, set when this packet contains the first
  // byte of a new payload unit.
  //      The first byte of the payload will indicate where this new payload
  //      unit starts.
  // TP   Transport priority, set when the current packet has a higher priority
  // than other packets with the same PID. PID  Packet Identifier, describing
  // the payload data. TSC  Transport scrambling control, '00' = Not scrambled.
  // AFC  Adaptation field control, 01 – no adaptation field, payload only, 10 –
  // adaptation field only, no payload,
  //                                11 – adaptation field followed by payload,
  //                                00 – RESERVED for future use
  // CC   Continuity counter, Sequence number of payload packets (0x00 to 0x0F)
  // within each stream (except PID 8191)

  if (packet[0] != 0x47) {
    log_e("ts SyncByte not found, first bytes are %X %X %X %X", packet[0],
  ...
  if (PID == 0) {
    ...
    int sectionLength =
        ((packet[PLS + 1] & 0x0F) << 8) | (packet[PLS + 2] & 0xFF);
    ...
    for (int i = startOfProgramNums; i <= sectionLength;
         i += lengthOfPATValue) {
      program_number =
          ((packet[PLS + i] & 0xFF) << 8) | (packet[PLS + i + 1] & 0xFF);
      program_map_PID =
          ((packet[PLS + i + 2] & 0x1F) << 8) | (packet[PLS + i + 3] & 0xFF);
      if (m_f_Log)
        log_i("Program Num: 0x%04X(%d) PMT PID: 0x%04X(%d)", program_number,
    ...
    *packetLength = 0;
    ...
    if (PES_DataLength > 0) {
      ...
      *packetLength = TS_PACKET_SIZE - posOfPacketStart - fillData;
      ...
      if (m_f_Log)
        log_i("First 3 bytes: %02X %02X %02X", firstByte, secondByte,
              thirdByte);
      if (firstByte == 0x00 && secondByte == 0x00 &&
          thirdByte == 0x01) {  // Packet start code prefix
        // PES
        ...
        int PES_PacketLength = ((packet[posOfPacketStart + 4] & 0xFF) << 8) +
                               (packet[posOfPacketStart + 5] & 0xFF);
        ...
        if (posOfPacketStart + startOfData >= 188) {  // only fillers in packet
          if (m_f_Log)
            log_e("posOfPacketStart + startOfData %i",
        ...
        if (m_f_Log)
          log_i("First AAC data byte: %02X",
                packet[posOfPacketStart + startOfData]);
        if (m_f_Log)
          log_i("Second AAC data byte: %02X",
    ...
    for (int i = 0; i < pidsOfPMT.number; i++) {
      if (PID == pidsOfPMT.pids[i]) {
        ...
        int programInfoLength =
            ((packet[PLS + 10] & 0x0F) << 8) | (packet[PLS + 11] & 0xFF);
        ...
        while (cursor < sectionLength - 1) {
          ...
          int elementaryPID = ((packet[PLS + cursor + 1] & 0x1F) << 8) |
                              (packet[PLS + cursor + 2] & 0xFF);
          if (m_f_Log)
            log_i("Stream Type: 0x%02X Elementary PID: 0x%04X", streamType,
                  elementaryPID);
          ...
          int esInfoLength = ((packet[PLS + cursor + 3] & 0x0F) << 8) |
                             (packet[PLS + cursor + 4] & 0xFF);
...
uint16_t Audio::readMetadata(uint16_t maxBytes, bool first) {
  ...
  if (metalen < m_chbufSize) {
    uint16_t a = _client->readBytes(
        &m_chbuf[pos_ml],
        min((uint16_t)(metalen - pos_ml), (uint16_t)(maxBytes - 1)));
    res += a;
    pos_ml += a;
  ...
  if (pos_ml == metalen) {
    ...
    if (strlen(m_chbuf)) {  // Any info present?
      ...
      int pos =
          indexOf(m_chbuf, "song_spot", 0);  // remove some irrelevant infos
...
size_t Audio::chunkedDataTransfer(uint8_t* bytes) {
  ...
  while (true) {
    ...
    if (b == '\n') break;
    if (b < '0') continue;
    // We have received a hexadecimal character.  Decode it and add to the
    // result.
...
bool Audio::readID3V1Tag() {
  ...
  // Version 1.x is a fixed size at the end of the file (128 bytes) after a
  // <TAG> keyword.
  if (m_codec != CODEC_MP3) return false;
  if (InBuff.bufferFilled() == 128 &&
      startsWith((const char*)InBuff.getReadPtr(),
                 "TAG")) {  // maybe a V1.x TAG
    char title[31];
  ...
  if (InBuff.bufferFilled() == 227 &&
      startsWith((const char*)InBuff.getReadPtr(),
                 "TAG+")) {  // ID3V1EnhancedTAG
...
boolean Audio::streamDetection(uint32_t bytesAvail) {
  ...
  // if within one second the content of the audio buffer falls below the size
  // of an audio frame 100 times, issue a message
  ...
  // if no audio data is received within three seconds, a new connection attempt
  // is started.
  if (tmr_lost < millis()) {
    cnt_lost++;
    tmr_lost = millis() + 1000;
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
uint32_t Audio::find_m4a_atom(uint32_t fileSize, const char* atomName,
                              uint32_t depth) {
  while (audiofile.position() < fileSize) {
    ...
    audiofile.read(
        (uint8_t*)&atomSize,
        4);  // Read the atom size (4 bytes) and the atom type (4 bytes)
    if (!atomSize)
      audiofile.read((uint8_t*)&atomSize, 4);  // skip 4 byte offset field
    audiofile.read((uint8_t*)atomType, 4);

    atomSize = bswap32(
        atomSize);  // Convert atom size from big-endian to little-endian
    ///    log_w("%*sAtom '%s' found at position %u with size %u bytes", depth *
    ///    2, "", atomType, atomStart, atomSize);
    if (strncmp(atomType, atomName, 4) == 0) return atomStart;

    if (atomSize ==
        1) {  // If the atom has a size of 1, an 'Extended Size' is used
      ...
      //    extendedSize);
      atomSize =
          (uint32_t)extendedSize;  // Limit to uint32_t for further processing
    }

    // If the atom is a container, read the atoms contained in it recursively
    if (strncmp(atomType, "moov", 4) == 0 ||
        strncmp(atomType, "trak", 4) == 0 ||
        strncmp(atomType, "mdia", 4) == 0 ||
        strncmp(atomType, "minf", 4) == 0 ||
        strncmp(atomType, "stbl", 4) == 0 ||
        strncmp(atomType, "meta", 4) == 0 ||
      ...
      audiofile.seek(atomStart +
                     atomSize);  // No container atom, jump to the next atom
    }
  }
  return 0;
}
...
void Audio::seek_m4a_stsz() {
  // stsz says what size each sample is in bytes. This is important for the
  // decoder to be able to start at a chunk, and then go through each sample by
  // its size. The stsz atom can be behind the audio block. Therefore, searching
  ...
  /* atom hierarchy
    ...
    the position and number of entries stco
  ...
  // c99 has no inner functions, lambdas are only allowed from c11, please don't
  // use ancient compiler
  ...
  for (int i = 0; i < 6; i++) {
    ...
    while (seekpos < at.pos + at.size) {
      ...
      seekpos += tmp.size;
      //  log_i("tmp.name %s, tmp.size %i, seekpos %i", tmp.name, tmp.size,
      //  seekpos);
      ...
      if (strcmp(tmp.name, "stsd") ==
          0) {  // in stsd we can found mp4a atom that contains the audioitems
        stsdPos = tmp.pos;
        stsdSize = tmp.size;
      }
    }
...
uint32_t Audio::m4a_correctResumeFilePos(uint32_t resumeFilePos) {
  // In order to jump within an m4a file, the exact beginning of an aac block
  // must be found. Since m4a cannot be streamed, i.e. there is no syncword, an
  // imprecise jump can lead to a crash.
...
int32_t Audio::mp3_correctResumeFilePos(uint32_t resumeFilePos) {
  // The SncronWord sequence 0xFF 0xF? can be part of valid audio data.
  // Therefore, it cannot be ensured that the next 0xFFF is really the beginning
  // of a new MP3 frame. Therefore, the following byte is parsed. If the bitrate
  // and sample rate match the one currently being played, the beginning of a
  // new MP3 frame is likely.
  ...
  auto find_sync_word = [&](size_t pos) -> int {
    ...
    while (audiofile.available()) {
      ...
      if (c == 0xFF) {  // First byte of sync word found
        ...
        steps++;
        if ((nextByte & 0xF0) ==
            0xF0) {          // Check for the second part of sync word
  ...
  while (true) {
    ...
    uint8_t mpegVers =
        (verIdx == 0 ? MPEG25 : ((verIdx & 0x01) ? MPEG1 : MPEG2));
    ...
    if (brIdx) {
      uint32_t bitrate =
          ((int32_t)bitrateTab[mpegVers][layer - 1][brIdx]) * 1000;
      uint32_t samplerate = samplerateTab[mpegVers][srIdx];
      //    log_e("%02X, %02X bitrate %i, samplerate %i", syncH, syncL, bitrate,
      //    samplerate);
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
// separate task for decoding and outputting the data. 'playAudioData()' is
// started periodically and fetches the data from the InBuffer. This ensures
// that the I2S-DMA is always sufficiently filled, even if the Arduino 'loop' is
// stuck.
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

void Audio::setAudioTaskCore(
    uint8_t coreID) {  // Recommendation:If the ARDUINO RUNNING CORE is 1, the
                       // audio task should be core 0 or vice versa
  if (coreID > 1) return;
  stopAudioTask();
  xSemaphoreTake(mutex_audioTask, 0.3 * configTICK_RATE_HZ);
